...
struct ShaderSource
{
    std::vector<CoByte> vertexShader;
    std::vector<CoByte> fragmentShader;
};
...
shaderSource()
{
    ...
    auto vertexShader   = compileShader(std::string(Shaders::Shader_slang.begin(), Shaders::Shader_slang.end()), "vsMain");
    auto fragmentShader = compileShader(std::string(Shaders::Shader_slang.begin(), Shaders::Shader_slang.end()), "psMain");
    ...
    return ShaderSource{ std::move(*vertexShader), std::move(*fragmentShader) };
}
...
bool
loadImage(std::span<const uint8_t> buffer, Image& image)
{
    int x, y, c;
...
int main()
{
    ...
    vertexShaderConfig.pSource     = shader->vertexShader.data();
    ...
    fragmentShaderConfig.pSource     = shader->fragmentShader.data();
    ...
    auto [texture, sampler] = createTexture(context.get(), Coral::Resources::uvtest_png);
